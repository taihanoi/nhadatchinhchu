Đây là js mã chatbot viết 

Vào readme.md

/* RealEstateAI_v7_FinalFix.js
   ✅ Fix: Input chữ màu đen (Không bị chìm trên nền trắng).
   ✅ Tags: Cập nhật các mốc diện tích 20, 30, 40, 50m2.
   ✅ UI Mobile: Dạng Sheet 70%, có Backdrop, né bàn phím iOS.
*/
...
(function() {
    ...
    const Brain = {
        ...
        learn: function() {
            ...
            console.log(`🧠 AI V7 đã học ${Brain.knowledge.length} đoạn dữ liệu.`);
        },
    ...
    // === 3. GIAO DIỆN (UI V7 - Mobile Sheet + Input Fix) ===
    const UI = {
        init: function() {
            const css = `
                /* Nút mở chat */
                ...
                .ai-chip { 
                    background: #e4e6eb; padding: 8px 14px; border-radius: 20px; 
                    font-size: 13px; white-space: nowrap; font-weight: 500; color: #050505; cursor: pointer; 
                }
                
                #ai-input-wrap { display: flex; gap: 10px; align-items: center; }
                
                /* FIX INPUT MÀU: Ép cứng màu chữ đen và nền trắng */
                #ai-input { 
                    flex: 1; padding: 12px; border-radius: 24px; border: 1px solid #ddd; 
                    outline: none; font-size: 16px; 
                    background-color: #ffffff !important; 
                    color: #1c1e21 !important; /* Chữ đen tuyệt đối */
                }
                #ai-input::placeholder { color: #888 !important; }
                #ai-input:focus { border-color: ${CONFIG.color}; }
            ...
            const html = `
                <div id="ai-root">
                    ...
                    <div id="ai-box">
                        ...
                        <div id="ai-body">
                            <div class="ai-msg ai-bot">Chào bạn! Nhập diện tích (VD: 50m2) để tính giá, hoặc chọn nhanh bên dưới nhé! 🏠</div>
                        </div>
                        <div id="ai-footer">
                            <div id="ai-chips">
                                <div class="ai-chip">🏠 20m2</div>
                                <div class="ai-chip">🏠 30m2</div>
                                <div class="ai-chip">🏠 40m2</div>
                                <div class="ai-chip">🏠 50m2</div>
                                <div class="ai-chip">🏪 Mặt phố</div>
                            </div>
            ...
            // Xử lý Tags (20m, 30m, 40m...)
            document.querySelectorAll('.ai-chip').forEach(c => {
                c.onclick = () => { 
                    let t = c.innerText.replace("🏠 ", "").replace("🏪 ", ""); // Lấy text
                    // Nếu bấm số, thêm chữ "Ngõ" để bot hiểu là tính giá ngõ
                    if(t.includes("20m2")) t = "Nhà trong ngõ 20m2";
                    else if(t.includes("30m2")) t = "Nhà trong ngõ 30m2";
                    else if(t.includes("40m2")) t = "Nhà trong ngõ 40m2";
                    else if(t.includes("50m2")) t = "Nhà trong ngõ 50m2";
                    else if(t.includes("Mặt phố")) t = "Nhà mặt phố kinh doanh";
